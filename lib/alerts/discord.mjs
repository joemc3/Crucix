  ...
  async start() {
    ...
    try {
      ...
      this._client.on('interactionCreate', async (interaction) => {
        ...
        // SECURITY: Role-based authorization for Discord bot commands (mitigates F8)
        // When DISCORD_AUTHORIZED_ROLES is set, only members with at least one listed role can execute commands.
        // When unset, all server members can use commands (backwards compat).
        const authorizedRoles = process.env.DISCORD_AUTHORIZED_ROLES;
        if (authorizedRoles) {
          const allowed = new Set(authorizedRoles.split(',').map(s => s.trim()));
          const memberRoles = [...(interaction.member?.roles?.cache?.keys() || [])];
          if (!memberRoles.some(r => allowed.has(r))) {
