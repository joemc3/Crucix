  ...
  async _pollUpdates() {
    try {
      ...
      for (const update of data.result) {
        ...
        // SECURITY: User-level authorization for bot commands (mitigates F7)
        // When TELEGRAM_AUTHORIZED_USERS is set, only listed user IDs can execute commands.
        // When unset, any member of the configured chat can use commands (backwards compat).
        const authorizedUsers = process.env.TELEGRAM_AUTHORIZED_USERS;
        if (authorizedUsers) {
          const allowed = new Set(authorizedUsers.split(',').map(s => s.trim()));
          if (!allowed.has(String(msg.from?.id))) continue;
        }
